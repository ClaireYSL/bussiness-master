...
M212 = MILESTONES / "milestone212r_static_promote_report_only"
M213 = MILESTONES / "milestone213r_trusted_pool_publish_l3"
...
def load_m212() -> dict[str, Any]:
    return {
        "report": read_json(M212 / "m212_static_promote_report_only_v1.json"),
        "baseline": read_json(M212 / "m212_report_baseline_v1.json"),
        "validation": read_json(M212 / "m212_validation_report_v1.json"),
        "expert": read_json(M212 / "m212_expert_review_report_v1.json"),
        "pool_diff": read_json(M212 / "m212_pool_diff_preview_v1.json"),
    }


def validate_preconditions(args: argparse.Namespace, m212: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    ...
    report = m212["report"]
    if m212["validation"].get("status") != "PASS":
        errors.append("M212 validation is not PASS")
    if m212["expert"].get("overall_review_status") != "pass":
        errors.append("M212 expert review is not pass")
    if m212["baseline"].get("candidate_signature") != report.get("candidate_signature"):
        errors.append("M212 baseline signature mismatch")
    if report.get("summary", {}).get("decision_counts", {}).get("allow", 0) != report.get("summary", {}).get("report_only_candidate_count"):
        errors.append("not all M212 candidates are allow")
    if report.get("summary", {}).get("suggested_level_counts", {}).get("L3", 0) != report.get("summary", {}).get("report_only_candidate_count"):
        errors.append("M212 candidates are not all L3")
...
def decision_by_id(m212: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {decision["prospect_id"]: decision for decision in m212["report"].get("decisions") or []}


def trace_preview_by_id(m212: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {item["prospect_id"]: item for item in m212["pool_diff"].get("source_trace_preview") or []}


def candidate_items(m212: dict[str, Any]) -> list[dict[str, Any]]:
    decisions = decision_by_id(m212)
    items = []
    for candidate in m212["report"].get("candidates") or []:
        ...
        item.update({
            ...
            "canonical_update_source": "M213R_publish_M212_L3_preview",
...
def update_pool(m212: dict[str, Any]) -> dict[str, Any]:
    ...
    added = updated = 0
    changes = []
    for candidate in candidate_items(m212):
...
def update_source_trace(m212: dict[str, Any]) -> dict[str, Any]:
    ...
    trace["summary"] = {**(trace.get("summary") or {}), "source_trace_count": len(items), "source_category_counts": dict(category_counts), "canonical_update_source": "M213R_publish_M212_L3_preview", "signed_customer_v2_gate_applied": True}
...
def update_entity_registry(m212: dict[str, Any]) -> dict[str, Any]:
    ...
    for signed in signed_registry.get("items") or []:
        ...
        if idx is None:
            items.append({
                ...
                "canonical_update_source": "M213R_sync_signed_customer_entities",
            })
            signed_added += 1
    for candidate in candidate_items(m212):
        entity = {
...
def write_vault_l3(m212: dict[str, Any]) -> dict[str, Any]:
...
def update_panel(pool_update: dict[str, Any], trace_update: dict[str, Any], vault_write: dict[str, Any], validation_status: str) -> None:
    ...
    counts.update({
        ...
        "m213_added_l3_count": pool_update.get("added_count"),
        "m213_updated_l3_count": pool_update.get("updated_count"),
        "m213_published_l3_count": (pool_update.get("added_count", 0) + pool_update.get("updated_count", 0)),
        "m213_vault_l3_written_count": vault_write.get("written_count"),
    })
    ...
    panel.update({
        "generated_at": now(),
        "latest_milestone": "M213R",
        "overall_status": "PASS_M213R_TRUSTED_POOL_L3_PUBLISH" if validation_status == "PASS" else "FAIL_M213R_TRUSTED_POOL_L3_PUBLISH",
        "counts": counts,
        "current_canonical_state": current,
        "m213r_trusted_pool_l3_publish": {"generated_at": now(), "status": validation_status, "pool_update": pool_update, "source_trace_update": trace_update, "vault_write_count": vault_write.get("written_count")},
        "canonical_next_action": "进入 M202：为 M213 新增 L3 补第二强来源，评估 L3 -> L2。",
    })
    write_json(PANEL, panel)


def validate(pool_update: dict[str, Any], trace_update: dict[str, Any], entity_update: dict[str, Any], vault_write: dict[str, Any], m212: dict[str, Any]) -> dict[str, Any]:
    pyc = run(["python3", "-m", "py_compile", "scripts/build_m213r_trusted_pool_publish_l3.py", "shared/static_pool/signed_customer_gate.py", "shared/static_pool/prospect_eligibility_gate.py"])
    json_errors = []
    for path in M213.glob("*.json"):
        try:
            json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
            json_errors.append({"file": rel(path), "error": str(exc)})
    dynamic = scan_dynamic([M213, L3_DIR, WORKSPACE / "scripts/build_m213r_trusted_pool_publish_l3.py"])
    api = scan_api([M213, L3_DIR, WORKSPACE / "scripts/build_m213r_trusted_pool_publish_l3.py"])
    expected = m212["report"].get("summary", {}).get("report_only_candidate_count")
    checks = {
        "py_compile_pass": pyc["returncode"] == 0,
        "json_parse_pass": not json_errors,
        "pool_published_count_matches_m212": (pool_update.get("added_count", 0) + pool_update.get("updated_count", 0)) == expected,
        "source_trace_published_count_matches_m212": (trace_update.get("added_count", 0) + trace_update.get("updated_count", 0)) == expected,
        "vault_written_count_matches_m212": vault_write.get("written_count") == expected,
    ...
    payload = {
        ...
        "checks": checks,
        "py_compile": pyc,
        "json_parse": {"checked_count": len(list(M213.glob("*.json"))), "errors": json_errors},
    ...
    write_json(M213 / "m213_validation_report_v1.json", payload)
    return payload


def build_all(args: argparse.Namespace) -> dict[str, Any]:
    M213.mkdir(parents=True, exist_ok=True)
    m212 = load_m212()
    errors = validate_preconditions(args, m212)
    if errors:
        payload = {"milestone": "M213R", "generated_at": now(), "status": "FAIL", "errors": errors}
        write_json(M213 / "m213_validation_report_v1.json", payload)
        return payload
    pool_update = update_pool(m212)
    trace_update = update_source_trace(m212)
    entity_update = update_entity_registry(m212)
    vault_write = write_vault_l3(m212)
    write_json(M213 / "canonical_update_diff_v1.json", {"milestone": "M213R", "generated_at": now(), "pool_update": pool_update, "source_trace_update": trace_update, "entity_update": entity_update, "vault_write": vault_write})
    validation = validate(pool_update, trace_update, entity_update, vault_write, m212)
    expert = {"milestone": "M213R", "generated_at": now(), "overall_review_status": "pass" if validation["status"] == "PASS" else "fail", "product_review": {"status": "pass" if validation["status"] == "PASS" else "fail", "notes": "M213 将 M212 通过的 L3 候选发布到用户可读 L3 摘要层；不把 L3 冒充 L2。"}, "architecture_review": {"status": "pass" if validation["status"] == "PASS" else "fail", "notes": "trusted pool/source trace/entity/vault 写入均由显式 guard 触发，并保留 diff。"}, "data_governance_review": {"status": "pass" if validation["status"] == "PASS" else "fail", "notes": "不写旧 Excel、knowledge asset registry、persona registry；signed customer v2 gate 已在上游通过。"}}
    write_json(M213 / "m213_expert_review_report_v1.json", expert)
    operating = {"milestone": "M213R", "generated_at": now(), "status": "PASS_M213R_TRUSTED_POOL_L3_PUBLISH" if validation["status"] == "PASS" else "FAIL_M213R_TRUSTED_POOL_L3_PUBLISH", "summary": {"pool_after_count": pool_update.get("after_count"), "source_trace_after_count": trace_update.get("after_count"), "added_l3_count": pool_update.get("added_count"), "updated_l3_count": pool_update.get("updated_count"), "vault_l3_written_count": vault_write.get("written_count"), "expert_review_status": expert.get("overall_review_status")}, "next_recommended_action": "M202：补第二强来源并评估 L3 -> L2。"}
    write_json(M213 / "m213_operating_panel_v1.json", operating)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Publish M212 L3 preview into canonical trusted pool and vault L3 cards.")
