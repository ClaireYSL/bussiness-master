...
M203 = MILESTONES / "milestone203r_publish_l2_upgrades"
...
def load_m202() -> dict[str, Any]:
    return {
        "report": read_json(M202 / "m202_l3_to_l2_report_only_v1.json"),
        "baseline": read_json(M202 / "m202_report_baseline_v1.json"),
        "validation": read_json(M202 / "m202_validation_report_v1.json"),
        "expert": read_json(M202 / "m202_expert_review_report_v1.json"),
        "trace_patch": read_json(M202 / "source_trace_patch_v1.json"),
        "pool_diff": read_json(M202 / "m202_pool_diff_preview_v1.json"),
    }


def validate_preconditions(args: argparse.Namespace, m202: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    report = m202["report"]
    ...
    if m202["validation"].get("status") != "PASS":
        errors.append("M202 validation is not PASS")
    if m202["expert"].get("overall_review_status") != "pass":
        errors.append("M202 expert review is not pass")
    if m202["baseline"].get("candidate_signature") != report.get("candidate_signature"):
        errors.append("M202 baseline signature mismatch")
    if report.get("summary", {}).get("suggested_level_counts", {}).get("L2", 0) <= 0:
        errors.append("M202 has no L2 preview candidates")
...
def l2_candidates(m202: dict[str, Any]) -> list[dict[str, Any]]:
    report = m202["report"]
    ...
    for candidate in report.get("candidates") or []:
        ...
        item.update({
            ...
            "canonical_update_source": "M203R_publish_M202_L2_preview",
...
def update_pool(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool["summary"] = {**(pool.get("summary") or {}), "trusted_pool_count": len(items), "static_level_counts": dict(after_levels), "canonical_update_source": "M203R_publish_M202_L2_preview", "old_workbook_write_enabled": False, "knowledge_asset_write_enabled": False, "persona_registry_write_enabled": False, "signed_customer_v2_gate_applied": True}
...
def update_trace(m202: dict[str, Any], candidates: list[dict[str, Any]]) -> dict[str, Any]:
    trace = read_json(CANONICAL_TRACE, {"items": []})
    before_hash = stable_hash(trace)
    items = trace.get("items") or []
    patch_by_id = by_id(m202["trace_patch"].get("items") or [])
    changes = []
    for candidate in candidates:
    ...
    trace["summary"] = {**(trace.get("summary") or {}), "source_trace_count": len(items), "source_category_counts": dict(category_counts), "canonical_update_source": "M203R_publish_M202_L2_preview", "signed_customer_v2_gate_applied": True}
...
def update_entity_registry(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        if idx is None:
            ...
            items[idx]["canonical_update_source"] = "M203R_publish_M202_L2_preview"
        updated += 1
    role_counts = Counter(role for row in items for role in row.get("entity_roles") or [])
    registry["items"] = items
    registry["generated_at"] = now()
    registry["summary"] = {**(registry.get("summary") or {}), "entity_count": len(items), "trusted_pool_entity_count": role_counts.get("trusted_pool", 0), "source_trace_entity_count": role_counts.get("source_trace", 0), "signed_customer_entity_count": role_counts.get("signed_customer", 0), "canonical_update_source": "M203R_publish_M202_L2_preview"}
...
def write_vault_l2(m202: dict[str, Any], candidates: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
...
def update_panel(pool_update: dict[str, Any], trace_update: dict[str, Any], vault_write: dict[str, Any], removal: dict[str, Any], validation_status: str) -> None:
    ...
    counts.update({
        ...
        "m203_l2_updated_count": pool_update.get("updated_count"),
        "m203_vault_l2_written_count": vault_write.get("written_count"),
        "m203_l3_card_removed_count": removal.get("removed_count"),
    })
    ...
    panel.update({
        "generated_at": now(),
        "latest_milestone": "M203R",
        "overall_status": "PASS_M203R_L2_PUBLISH" if validation_status == "PASS" else "FAIL_M203R_L2_PUBLISH",
        "counts": counts,
        "current_canonical_state": current,
        "m203r_l2_publish": {"generated_at": now(), "status": validation_status, "pool_update": pool_update, "source_trace_update": trace_update, "vault_l2_written_count": vault_write.get("written_count"), "l3_card_removed_count": removal.get("removed_count")},
        "canonical_next_action": "进入 M204：完成本轮生产闭环复盘，并决定继续补 L1 证据链或启动下一轮候选发现。",
    })
    write_json(PANEL, panel)


def validate(pool_update: dict[str, Any], trace_update: dict[str, Any], entity_update: dict[str, Any], vault_write: dict[str, Any], removal: dict[str, Any], index_update: dict[str, Any], m202: dict[str, Any]) -> dict[str, Any]:
    pyc = run(["python3", "-m", "py_compile", "scripts/build_m203r_publish_l2_upgrades.py", "shared/static_pool/signed_customer_gate.py"])
    json_errors = []
    for path in M203.glob("*.json"):
        try:
            json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
            json_errors.append({"file": rel(path), "error": str(exc)})
    dynamic = scan_dynamic([M203, *[Path(item["path"]) for item in vault_write.get("items") or []], L2_INDEX, L3_INDEX, WORKSPACE / "scripts/build_m203r_publish_l2_upgrades.py"])
    api = scan_api([M203, *[Path(item["path"]) for item in vault_write.get("items") or []], WORKSPACE / "scripts/build_m203r_publish_l2_upgrades.py"])
    expected = m202["report"].get("summary", {}).get("suggested_level_counts", {}).get("L2", 0)
    old_hits = 0
    ...
    payload = {"milestone": "M203R", "generated_at": now(), "status": "PASS" if all(checks.values()) else "FAIL", "checks": checks, "py_compile": pyc, "json_parse": {"checked_count": len(list(M203.glob("*.json"))), "errors": json_errors}, "dynamic_term_scan": dynamic, "api_key_scan": api, "no_write_proof": {"old_excel_written": False, "knowledge_asset_registry_written": False, "persona_registry_written": False, "trusted_pool_written": True, "canonical_source_trace_written": True, "vault_regular_area_written": True}}
    write_json(M203 / "m203_validation_report_v1.json", payload)
    return payload
...
def build_expert_review(validation: dict[str, Any], pool_update: dict[str, Any], vault_write: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "product_review": {"status": "pass" if passed else "fail", "notes": "M203 将 M202 已通过的 19 家 L2 preview 正式写入 L2 用户阅读层；未通过对象继续保留 L3。"},
    ...
    write_json(M203 / "m203_expert_review_report_v1.json", payload)
    return payload


def build_all(args: argparse.Namespace) -> dict[str, Any]:
    M203.mkdir(parents=True, exist_ok=True)
    m202 = load_m202()
    errors = validate_preconditions(args, m202)
    if errors:
        payload = {"milestone": "M203R", "generated_at": now(), "status": "FAIL_MISSING_OR_INVALID_GUARD", "errors": errors}
        write_json(M203 / "m203_validation_report_v1.json", payload)
        return payload
    candidates = l2_candidates(m202)
    pool_update = update_pool(candidates)
    trace_update = update_trace(m202, candidates)
    entity_update = update_entity_registry(candidates)
    vault_write, removal = write_vault_l2(m202, candidates)
    index_update = refresh_indexes()
    diff = {"milestone": "M203R", "generated_at": now(), "pool_update": pool_update, "source_trace_update": trace_update, "entity_update": entity_update, "vault_l2_write": vault_write, "l3_card_removal": removal, "index_update": index_update}
    write_json(M203 / "canonical_update_diff_v1.json", diff)
    write_json(M203 / "l3_card_removal_manifest_v1.json", {"milestone": "M203R", "generated_at": now(), **removal})
    validation = validate(pool_update, trace_update, entity_update, vault_write, removal, index_update, m202)
    expert = build_expert_review(validation, pool_update, vault_write)
    operating = {"milestone": "M203R", "generated_at": now(), "status": "PASS_M203R_L2_PUBLISH" if validation["status"] == "PASS" else "FAIL_M203R_L2_PUBLISH", "summary": {"trusted_pool_count": pool_update.get("after_count"), "level_counts": pool_update.get("after_levels"), "l2_updated_count": pool_update.get("updated_count"), "vault_l2_written_count": vault_write.get("written_count"), "l3_card_removed_count": removal.get("removed_count"), "expert_review_status": expert.get("overall_review_status")}, "next_recommended_action": "M204：完成本轮生产闭环复盘，并决定继续补 L1 证据链或启动下一轮候选发现。"}
    write_json(M203 / "m203_operating_panel_v1.json", operating)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Publish M202 L2 previews into canonical trusted pool and vault L2 dossiers.")
