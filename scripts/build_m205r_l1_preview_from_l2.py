...
THIRD_SOURCE_PLAN = {
    "杭州花西子化妆品股份有限公司": {"source_type": "official_product_collection", "source_category": "platform_operating_fact", "source_locator": "https://www.florasis.com/collections/all", "supports_dimension": "icp_match_support:brand_product_matrix,sku_matrix,channel_complexity"},
    "上海林清轩生物科技有限公司": {"source_type": "official_product_page", "source_category": "platform_operating_fact", "source_locator": "https://www.lqxshop.com/products", "supports_dimension": "icp_match_support:brand_product_matrix,sku_matrix,channel_complexity"},
    "上海新锐供应链管理有限公司": {"source_type": "official_contact_page", "source_category": "official_owned", "source_locator": "https://www.cupshe.com/pages/contact-us", "supports_dimension": "icp_match_support:global_brand_operations,cross_border_operations,multi_region"},
    "深圳市泽宝创新技术有限公司": {"source_type": "official_product_collection", "source_category": "platform_operating_fact", "source_locator": "https://www.ravpower.com/collections/all", "supports_dimension": "icp_match_support:consumer_hardware_brand,sku_matrix,cross_border_operations"},
    "深圳市蓝禾技术有限公司": {"source_type": "official_product_collection", "source_category": "platform_operating_fact", "source_locator": "https://www.benks.com/collections/all", "supports_dimension": "icp_match_support:consumer_accessory_brand,sku_matrix,cross_border_operations"},
    "深圳市时商创展科技有限公司": {"source_type": "official_product_collection", "source_category": "platform_operating_fact", "source_locator": "https://www.omoton.com/collections/all", "supports_dimension": "icp_match_support:consumer_accessory_brand,sku_matrix,cross_border_operations"},
    "广州棒谷科技股份有限公司": {"source_type": "official_wholesale_page", "source_category": "platform_operating_fact", "source_locator": "https://www.banggood.cn/Wholesale.html", "supports_dimension": "icp_match_support:platform_operator,multi_platform_commerce,cross_border_operations"},
    ...
    "广州影石创新科技有限公司": {"source_type": "official_support_page", "source_category": "official_owned", "source_locator": "https://www.insta360.com/support", "supports_dimension": "icp_match_support:consumer_hardware_brand,global_brand_operations,multi_region"},
    "深圳市大疆创新科技有限公司": {"source_type": "official_product_page", "source_category": "platform_operating_fact", "source_locator": "https://www.dji.com/products", "supports_dimension": "icp_match_support:consumer_hardware_brand,sku_matrix,global_brand_operations"},
    "追觅科技（苏州）有限公司": {"source_type": "official_product_collection", "source_category": "platform_operating_fact", "source_locator": "https://www.dreame.tech/collections/all", "supports_dimension": "icp_match_support:consumer_hardware_brand,sku_matrix,global_brand_operations"},
    "添可智能科技有限公司": {"source_type": "official_about_page", "source_category": "official_owned", "source_locator": "https://www.tineco.com/about", "supports_dimension": "icp_match_support:consumer_hardware_brand,global_brand_operations,channel_complexity"},
    "北京元气森林饮料有限公司": {"source_type": "official_about_page", "source_category": "official_owned", "source_locator": "https://www.yuanqisenlin.com/about", "supports_dimension": "icp_match_support:retail_high_sku_brand,sku_matrix,channel_complexity"},
}
...
def load_targets() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    targets = [item for item in pool.get("items") or [] if item.get("level") == "L2" and item.get("company_name") in THIRD_SOURCE_PLAN]
...
def build_patch() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    patch = {"package_id": "m205r_third_source_patch_package_v1", "milestone": "M205R", "generated_at": now(), "summary": {"target_l2_count": len(targets), "third_source_ready_count": status_counts.get("third_source_ready", 0), "source_health_gap_count": status_counts.get("source_health_gap", 0), "customer_case_used_as_prospect_evidence": False, "llm_used_as_evidence": False, "trusted_pool_written": False}, "items": patch_items}
    trace = {"index_id": "m205r_source_trace_patch_v1", "milestone": "M205R", "generated_at": now(), "summary": {"candidate_count": len(trace_items), "third_source_ready_count": status_counts.get("third_source_ready", 0), "canonical_source_trace_written": False}, "items": trace_items}
    write_json(M205 / "third_source_patch_package_v1.json", patch)
    write_json(M205 / "source_trace_patch_v1.json", trace)
    return patch, trace
...
def build_report_only(patch: dict[str, Any], trace_patch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    report = {"batch_id": "m205r_l1_preview_report_only_v1", "milestone": "M205R", "generated_at": now(), "mode": "report_only", "candidate_signature": signature, "summary": {"report_only_candidate_count": len(candidates), "decision_counts": dict(decision_counts), "suggested_level_counts": dict(level_counts), "l1_preview_count": level_counts.get("L1", 0), "l2_remaining_count": level_counts.get("L2", 0), "gap_queue_count": len(gap_queue), "canonical_pool_updated": False, "canonical_source_trace_written": False, "vault_regular_area_written": False}, "candidates": candidates, "decisions": decisions, "gap_queue": gap_queue, "no_write_proof": {"old_excel_written": False, "knowledge_asset_registry_written": False, "persona_registry_written": False, "trusted_pool_written": False, "canonical_source_trace_written": False, "vault_regular_area_written": False}}
    baseline = {"milestone": "M205R", "generated_at": now(), "candidate_signature": signature, "report_file": "m205_l1_preview_report_only_v1.json"}
    diff = {"diff_id": "m205r_pool_diff_preview_v1", "milestone": "M205R", "generated_at": now(), "summary": {"l1_preview_count": level_counts.get("L1", 0), "l2_remaining_count": level_counts.get("L2", 0), "canonical_pool_updated": False}, "items": [{"prospect_id": d["prospect_id"], "company_name": d["company_name"], "current_level": d["current_level"], "suggested_level": d["suggested_level"], "decision": d["decision"], "canonical_pool_updated": False} for d in decisions]}
    write_json(M205 / "m205_l1_preview_report_only_v1.json", report)
    write_json(M205 / "m205_report_baseline_v1.json", baseline)
    write_json(M205 / "m205_gap_queue_v1.json", {"milestone": "M205R", "generated_at": now(), "summary": {"gap_queue_count": len(gap_queue), "by_queue_type": dict(Counter(g.get("queue_type") for g in gap_queue)), "by_field": dict(Counter(g.get("field") for g in gap_queue))}, "items": gap_queue})
    write_json(M205 / "m205_pool_diff_preview_v1.json", diff)
...
def validate(patch: dict[str, Any], report: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    pyc = run(["python3", "-m", "py_compile", "scripts/build_m205r_l1_preview_from_l2.py", "shared/static_pool/static_promote.py"])
    json_errors = []
    for path in M205.glob("*.json"):
        try:
            json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
            json_errors.append({"file": rel(path), "error": str(exc)})
    dynamic = scan_dynamic([M205, WORKSPACE / "scripts/build_m205r_l1_preview_from_l2.py"])
    api = scan_api([M205, WORKSPACE / "scripts/build_m205r_l1_preview_from_l2.py"])
    ...
    payload = {"milestone": "M205R", "generated_at": now(), "status": "PASS" if all(checks.values()) else "FAIL", "checks": checks, "py_compile": pyc, "json_parse": {"checked_count": len(list(M205.glob("*.json"))), "errors": json_errors}, "dynamic_term_scan": dynamic, "api_key_scan": api, "no_write_proof": report.get("no_write_proof")}
    write_json(M205 / "m205_validation_report_v1.json", payload)
    return payload


def build_expert_review(validation: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    passed = validation.get("status") == "PASS"
    payload = {"milestone": "M205R", "generated_at": now(), "overall_review_status": "pass" if passed else "fail", "product_review": {"status": "pass" if passed else "fail", "notes": "M205 只形成 L1 preview，不把 L2 自动改成 L1；用户入口暂不变化。"}, "architecture_review": {"status": "pass" if passed else "fail", "notes": "第三来源 patch、source trace patch、report-only、baseline、pool diff preview 分离，未写 canonical。"}, "data_governance_review": {"status": "pass" if passed else "fail", "notes": "第三来源来自公开可定位来源；客户案例与 LLM 均未作为 evidence；不写旧 Excel、knowledge/persona registry。"}, "report_summary": report.get("summary")}
    write_json(M205 / "m205_expert_review_report_v1.json", payload)
    return payload
...
def update_panel(report: dict[str, Any], patch: dict[str, Any], validation_status: str) -> None:
    ...
    levels = report.get("summary", {}).get("suggested_level_counts") or {}
    counts.update({"m205_report_only_candidate_count": report.get("summary", {}).get("report_only_candidate_count"), "m205_l1_preview_count": levels.get("L1", 0), "m205_l2_remaining_count": levels.get("L2", 0), "m205_third_source_ready_count": patch.get("summary", {}).get("third_source_ready_count"), "m205_source_health_gap_count": patch.get("summary", {}).get("source_health_gap_count"), "m205_gap_queue_count": report.get("summary", {}).get("gap_queue_count")})
    panel.update({"generated_at": now(), "latest_milestone": "M205R", "overall_status": "PASS_M205R_L1_PREVIEW_FROM_L2" if validation_status == "PASS" else "FAIL_M205R_L1_PREVIEW_FROM_L2", "counts": counts, "m205r_l1_preview_from_l2": {"generated_at": now(), "status": validation_status, "suggested_level_counts": levels, "decision_counts": report.get("summary", {}).get("decision_counts"), "third_source_summary": patch.get("summary")}, "canonical_next_action": "进入 M206：对 M205 L1 preview 执行 guarded trusted pool/source trace/vault L1 更新，或先人工抽检 L1 证据链。"})
    write_json(PANEL, panel)


def build_all() -> dict[str, Any]:
    M205.mkdir(parents=True, exist_ok=True)
    ...
    operating = {"milestone": "M205R", "generated_at": now(), "status": "PASS_M205R_L1_PREVIEW_FROM_L2" if validation["status"] == "PASS" else "FAIL_M205R_L1_PREVIEW_FROM_L2", "summary": {**report.get("summary", {}), "third_source_summary": patch.get("summary"), "expert_review_status": expert.get("overall_review_status")}, "next_recommended_action": "M206：guarded update L1 preview；或先人工抽检 L1 证据链。"}
    write_json(M205 / "m205_operating_panel_v1.json", operating)
    update_panel(report, patch, validation["status"])
    return {"status": validation["status"], "summary": operating["summary"]}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build M205 L1 preview from selected L2 prospects with third-source patch.")
