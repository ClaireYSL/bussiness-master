...
REPORT_INPUT = M191 / "report_only_ready_input_v1.json"
SOURCE_TRACE_DRAFT = M191 / "source_trace_package_v1.json"
...
def build_candidates() -> tuple[list[dict[str, Any]], dict[str, list[dict[str, Any]]]]:
    ...
    for item in report_input.get("items") or []:
        prospect_id = stable_id("m192", str(item.get("candidate_name") or item.get("task_id")))
        ...
        candidate = {
            ...
            "candidate_source": "M191R_report_only_ready_input",
        }
...
def build_report_only() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    report = {
        "batch_id": "m192r_static_promote_report_only_v1",
        "milestone": "M192R",
        "generated_at": now(),
        "mode": "report_only",
    ...
    baseline = {"milestone": "M192R", "generated_at": now(), "candidate_signature": candidate_signature, "report_file": "m192_static_promote_report_only_v1.json"}
    pool_diff = {
        "diff_id": "m192r_pool_diff_preview_v1",
        "milestone": "M192R",
        "generated_at": now(),
        "mode": "preview_only",
        "summary": {
    ...
    write_json(M192 / "m192_static_promote_report_only_v1.json", report)
    write_json(M192 / "m192_report_baseline_v1.json", baseline)
    write_json(M192 / "m192_gap_queue_v1.json", {"milestone": "M192R", "generated_at": now(), "summary": {"gap_queue_count": len(gap_queue), "by_queue_type": dict(Counter(gap.get("queue_type") for gap in gap_queue))}, "items": gap_queue})
    write_json(M192 / "m192_pool_diff_preview_v1.json", pool_diff)
...
def build_gate_report(report: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    write_json(M192 / "m192_gate_check_v1.json", payload)
    return payload
...
def update_panel(report: dict[str, Any], validation_status: str) -> None:
    ...
    counts.update({
        "m192_report_only_candidate_count": summary.get("report_only_candidate_count"),
        "m192_l3_candidate_count": levels.get("L3", 0),
        "m192_l2_candidate_count": levels.get("L2", 0),
        "m192_l1_candidate_count": levels.get("L1", 0),
        "m192_gap_queue_count": summary.get("gap_queue_count"),
    })
    panel.update({
        "generated_at": now(),
        "latest_milestone": "M192R",
        "overall_status": "PASS_M192R_STATIC_PROMOTE_REPORT_ONLY" if validation_status == "PASS" else "FAIL_M192R_STATIC_PROMOTE_REPORT_ONLY",
        "counts": counts,
        "m192r_static_promote_report_only": {
            "generated_at": now(),
        ...
        "canonical_next_action": "进入 M193：基于 M192 pool diff preview 执行 guarded trusted pool update，或先补第二强来源提升 L2。",
    })
...
def validate(report: dict[str, Any], baseline: dict[str, Any], pool_diff: dict[str, Any], gate_report: dict[str, Any]) -> dict[str, Any]:
    pyc = run(["python3", "-m", "py_compile", "scripts/build_m192r_static_promote_report_only.py", "shared/static_pool/static_promote.py", "shared/static_pool/signed_customer_gate.py", "shared/static_pool/prospect_eligibility_gate.py"])
    json_errors = []
    for path in M192.glob("*.json"):
        try:
            json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
            json_errors.append({"file": rel(path), "error": str(exc)})
    dynamic = scan_dynamic([M192, WORKSPACE / "scripts/build_m192r_static_promote_report_only.py"])
    api = scan_api([M192, WORKSPACE / "scripts/build_m192r_static_promote_report_only.py"])
    ...
    payload = {
        ...
        "checks": checks,
        "py_compile": pyc,
        "json_parse": {"checked_count": len(list(M192.glob("*.json"))), "errors": json_errors},
        "dynamic_term_scan": dynamic,
        "api_key_scan": api,
        "no_write_proof": report.get("no_write_proof"),
    }
    write_json(M192 / "m192_validation_report_v1.json", payload)
    return payload
...
def build_expert_review(validation: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "product_review": {"status": "pass" if passed else "fail", "notes": "M192 将 M191 候选评估为静态可信摘要层，不硬升 L2；用户仍不会看到未发布对象。"},
        ...
        "data_governance_review": {"status": "pass" if passed else "fail", "notes": "signed customer v2 / duplicate gate 已在 M191 前置，本轮不写旧 Excel、knowledge/persona registry、trusted pool 或 vault。"},
        "report_summary": report.get("summary"),
    }
    write_json(M192 / "m192_expert_review_report_v1.json", payload)
    return payload


def build_all() -> dict[str, Any]:
    M192.mkdir(parents=True, exist_ok=True)
    ...
    operating = {
        "milestone": "M192R",
        "generated_at": now(),
        "status": "PASS_M192R_STATIC_PROMOTE_REPORT_ONLY" if validation["status"] == "PASS" else "FAIL_M192R_STATIC_PROMOTE_REPORT_ONLY",
        "summary": {**report.get("summary", {}), "expert_review_status": expert.get("overall_review_status")},
        "next_recommended_action": "M193：可选择先将 L3 preview guarded update 到 trusted pool，或补第二强来源后再冲 L2。",
    }
    write_json(M192 / "m192_operating_panel_v1.json", operating)
    update_panel(report, validation["status"])
    return {"status": validation["status"], "summary": operating["summary"]}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build M192 static promote report-only from M191 report-only-ready input.")
