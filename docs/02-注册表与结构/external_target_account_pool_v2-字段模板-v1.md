...
本文件定义 legacy Excel 版 `external_target_account_pool_v2` 的字段结构、判断字段、成熟度字段和治理字段。

M40R/M59R 后，本文件不再是新 evidence-first 可信潜客池的权威 schema。新默认事实承载应以 `trusted_prospect_pool_v1`、source trace index 和 vault 新区为准；本表仅作 legacy reference、去重、审计和历史兼容。
...
- md 客户档案页只是阅读投影，不是事实源；legacy Excel 修复时应回写其结构化层，新 evidence-first 可信池则应更新 `trusted_prospect_pool_v1` 与 source trace。
...
这只适用于 legacy Excel 工作簿治理，意味着：

- legacy 主表存结论
- legacy 档案库存事实
- legacy evidence 存依据
- md 页只是阅读层
- legacy 页面修复不能直接改 md 文案，而要先回写 legacy Excel 结构化层
- 新 evidence-first 可信池不得把旧主表、旧档案库、旧共享版当默认事实源

### `static_priority`

- `A`：legacy 内部治理排序最高，便于安排补证、解释和评估上移
- `B`：当前层内合理保留，但优先级次一档
...
- 它只回答 legacy 内部治理资源排序，不参与用户可见 L1-L5 定义
- 它不代表成交概率
- 不代表动态机会强弱
- 不代表动态经营排序
- 它主要综合以下因素：
